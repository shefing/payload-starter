      ...
      if (config.collections !== undefined) {
        config.collections
          ...
          .forEach((currentCollection) => {
            ...
            currentCollection.fields=processFields(fields, ((currentCollection as CollectionConfig)?.versions as IncomingCollectionVersions)?.drafts as boolean)
      ...
      if (config.globals !== undefined) {
        config.globals
          ...
          .forEach((globalConfig) => {
            ...
            globalConfig.fields = processFields(globalConfig.fields, ((globalConfig as GlobalConfig)?.versions as IncomingCollectionVersions)?.drafts as boolean)
          })
      }
      return config
    }
const processFields = (
  ...
  const hiddenFields = fields.filter((field) => (field as FieldAffectingData).admin?.hidden === true)
